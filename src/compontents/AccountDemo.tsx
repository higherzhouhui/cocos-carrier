...
const AccountDemo: React.FC = () => {
  const { isConnected, getAccountByChainId, getWalletSyncIsCompleted } =
    useConnectWallet();
  const [aelfAccount, setAelfAccount] = useState<string>("");
  const [tdvwAccount, setTdvwAccount] = useState<string>("");
  const [syncIsCompleted, setSyncIsCompleted] = useState<string | boolean>(
    false
  );
  const [syncIsCompletedTDVW, setSyncIsCompletedTDVW] = useState<
    string | boolean
  >(false);
  ...
  return (
    <div>
      <Button
        ...
        type="primary"
        onClick={onGetAccountByAELFHandler}>
      ...
      <Button
        disabled={!isConnected}
        type="primary"
        onClick={onGetAccountBytDVWHandler}>
      ...
      <Button
        disabled={!isConnected}
        type="primary"
        onClick={onGetWalletSyncIsCompletedHandler}>
